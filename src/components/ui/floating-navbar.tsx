...
export const FloatingNav = ({
  ...
  useMotionValueEvent(scrollYProgress, "change", (current) => {
    if (typeof current === "number") {
      ...
      setVisible(scrollY < windowHeight || direction < 0);
    }
  });
    ...
    <>
      <AnimatePresence mode="wait">
        <motion.div
          ...
          className={cn(
            "flex fixed top-4 inset-x-0 mx-auto border border-transparent dark:border-white/[0.2] rounded-full dark:bg-black bg-white shadow z-[50] px-4 py-2 items-center justify-between w-[95%] sm:max-w-2xl",
            className
          )}
        >
          {showLogo && (
            <div className="flex items-center gap-2 font-bold">
              ...
              <span className="text-sm">SkillSwap</span>
            </div>
          )}
            ...
            <div className="flex items-center justify-center space-x-5 mx-auto">
              {navItems.map((navItem, idx) => (
                <Link
                  ...
                  href={navItem.link}
                  className="relative dark:text-neutral-50 flex space-x-1 text-neutral-600 dark:hover:text-neutral-300 hover:text-neutral-500 px-2"
                >
          ...
          >
            ...
            <div className="flex flex-col space-y-1.5 pt-1">
              {navItems.map((navItem, idx) => (
                ...
                >
                  {navItem.icon && <span className="text-neutral-500 dark:text-neutral-400">{navItem.icon}</span>}
